...
class CheckerBoard:
    ...
    def peek_move(self, move):
        ...
        B.quiet_turns = 0 if move < 0 else (self.quiet_turns + 1)
    ...
    def get_move_sequences(self):
        ...
        move_sequences = []
        for move in moves:
            next_state = self.peek_move(move)
            if next_state.jump:
                next_sequence = next_state.get_move_sequences()
                sequence = [[move] + seq for seq in next_sequence]
                move_sequences.extend(sequence)
            else:
    ...
    def is_draw(self):
        return self.quiet_turns >= MAX_QUIET_TURNS

    def is_over(self):
        return self.is_draw() or len(self.get_moves()) == 0
